...
class TrackManager extends ChangeNotifier
{
 ...
 Future<void> loadFromFirebase() async
 {
  ...
  if (cloudTracks.isNotEmpty) 
  {
   _tracks = cloudTracks;
   notifyListeners();
   await _saveLocal(); // Keep the local phone storage in sync
  }

  // 2. Update local list
  _tracks = cloudTracks;

  // 3. Save to Local Storage so it's available offline
  await _saveLocal();

  notifyListeners();
 }

 // Clear tracks on Logout (Very important so User B doesn't see User A's tracks)
 void clearData() async
 {
  _tracks = [];
  notifyListeners();
  await _storage.saveTracks([]); // Wipe local disk
 }
